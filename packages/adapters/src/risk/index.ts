...
export class RiskManager {
  ...
  async getSystemHealth(): Promise<{
    ...
    const determineOverallHealth = (score: number) => {
      if (score >= 80) return 'healthy';
      if (score >= 60) return 'degraded';
      return 'critical';
    };

    const determineComponentHealth = (score: number) => {
      if (score >= 80) return 'healthy';
      if (score >= 60) return 'degraded';
      return 'down';
    };
    ...
    return {
      overall: determineOverallHealth(riskMetrics.overallScore),
      components: {
        api: apiHealth?.overallHealth || 'healthy',
        data: determineComponentHealth(systemData.dataQuality.defillama?.accuracy || 100),
        security: securitySummary.overallScore >= 80 ? 'healthy' : 'degraded',
        performance: determineComponentHealth(100 - systemData.performance.errorRate * 1000)
      },
      metrics: {
  ...
  // Cleanup
  destroy(): void {
    this.monitoring = undefined as any;
    this.apiReliability = undefined as any;
    this.financialAnalyzer = undefined as any;
    this.security = undefined as any;
