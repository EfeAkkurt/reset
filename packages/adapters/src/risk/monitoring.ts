...
export class RiskMonitoringService {
  ...
  private identifyRiskFactors(risks: any): import('@shared/core').RiskFactor[] {
    const factors: import('@shared/core').RiskFactor[] = [];

    if (risks.technical > 50) factors.push({
      category: 'technical',
      severity: 'high',
      description: 'API Reliability Issues',
      impact: risks.technical / 100,
      probability: 0.7,
      mitigation: 'Implement fallback mechanisms and monitoring'
    });
    if (risks.operational > 50) factors.push({
      category: 'operational',
      severity: 'medium',
      description: 'Performance Degradation',
      impact: risks.operational / 100,
      probability: 0.6,
      mitigation: 'Optimize performance and add caching'
    });
    if (risks.financial > 50) factors.push({
      category: 'financial',
      severity: 'high',
      description: 'Data Accuracy Concerns',
      impact: risks.financial / 100,
      probability: 0.8,
      mitigation: 'Validate data sources and implement checksums'
    });
    if (risks.security > 50) factors.push({
      category: 'security',
      severity: 'critical',
      description: 'Security Vulnerabilities',
      impact: risks.security / 100,
      probability: 0.5,
      mitigation: 'Security audit and implement additional safeguards'
    });
