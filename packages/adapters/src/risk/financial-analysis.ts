...
export class FinancialRiskAnalyzer {
  ...
  private generateRiskAssessment(params: {
    ...
    return {
      id: `risk-${opportunity.protocol}-${opportunity.pool}`,
      ...
      level,
      categories: {
        smartContractRisk: riskFactors.smartContractRisk,
        liquidityRisk: riskFactors.liquidityRisk,
        marketRisk: riskFactors.marketVolatility,
        counterpartyRisk: 0.1,
        operationalRisk: 0.1
      },
      factors: drivers.map(driver => ({
        category: 'market',
      ...
      recommendations: level === 'high' ? ['Consider reducing exposure', 'Monitor closely'] : ['Maintain monitoring'],
      lastUpdated: Date.now(),
      timestamp: Date.now(),
      confidence: (marketRegime.confidence >= 0.8 ? 'high' : marketRegime.confidence >= 0.5 ? 'medium' : 'low') as 'high' | 'medium' | 'low'
    };
  }
