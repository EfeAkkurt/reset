...
export class APIReliabilityManager {
  ...
  private async executeFastest(
    ...
    try {
      // Custom implementation of Promise.any for compatibility
      ...
      const fulfilledResult = results.find((r): r is { status: 'fulfilled', value: any } => r.status === 'fulfilled');
      if (fulfilledResult) {
        return fulfilledResult.value.result;
      }
